...
class WaypointUpdater(object):
    def __init__(self):
        ...
        self.subs['/current_pose'] = rospy.Subscriber('/current_pose',
                                                    PoseStamped, self.pose_cb)

        self.subs['/base_waypoints'] = rospy.Subscriber('/base_waypoints', 
                                                      Lane, self.waypoints_cb)

        self.subs['/current_velocity'] = rospy.Subscriber('/current_velocity',
                                                TwistStamped, self.velocity_cb)

        # TODO: Add a subscriber for /traffic_waypoint and /obstacle_waypoint 
        #  self.subs['/traffic_waypoint'] = rospy.Subscriber('/traffic_waypoint',
        #                                              String, self.traffic_cb)
        ...
        # rospy.wait_for_service("/waypoint_updater/set_parameters")
	    # client = dynamic_reconfigure.client.Client("waypoint_updater", 
        #                  timeout=30, config_callback=self.dyn_vars_cb)
...
#            rospy.set_param("~updater_rate", UPDATER_RATE)
        ...
        # rospy.spin()
        # change from publishing waypoint data every time new pose data is
        # received to publishing it on a fixed schedule. 

        self.loop()

    def loop(self):
        # self.update_rate = rospy.get_param("~updater_rate", UPDATER_RATE)
    ...
    def dyn_vars_cb(self, config, level):
        ...
        if old_default_velocity != config['dyn_default_velocity']:
            rospy.loginfo("waypoint_updater:dyn_vars_cb Adjusting default_"
                          "velocity from {} to {}".format(old_default_velocity,
        ...
        if old_lookahead_wps != config['dyn_lookahead_wps']:
            rospy.loginfo("waypoint_updater:dyn_vars_cb Adjusting lookahead_"
                          "wps from {} to {}".format(old_lookahead_wps,
    ...
    def send_waypoints(self):
        ...
        # for now assume loop
        #default_velocity = rospy.get_param("~default_velocity", 
        #                                               DEFAULT_VELOCITY)
        #lookahead_wps = rospy.get_param("~lookahead_wps", LOOKAHEAD_WPS)

        new_wps_list = []
        start_wps_ptr = self.closest_waypoint()
        end_wps_ptr = (start_wps_ptr + self.lookahead_wps) % len(self.waypoints)
